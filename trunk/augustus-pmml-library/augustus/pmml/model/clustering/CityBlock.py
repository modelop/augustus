...
"""This module defines the CityBlock class."""

from augustus.core.NumpyInterface import NP
from augustus.pmml.model.clustering.PmmlClusteringMetric import PmmlClusteringMetric

class CityBlock(PmmlClusteringMetric):
    """The CityBlock metric returns the sum of its components, an L-1 norm.  Also known as taxi-cab metric and Manhattan metric."""
    ...
    def initialize(self, state, numberOfRecords, numberOfFields, distributionBased):
        ...
        state.simpleSum = NP("zeros", numberOfRecords, dtype=NP.dtype(float))
    ...
    def accumulate(self, state, cxy, fieldWeight, distributionBased):
        ...
        NP("add", state.simpleSum, NP(cxy * fieldWeight), state.simpleSum)
    ...
    def finalizeDistance(self, state, adjustM, distributionBased, covarianceMatrix):
        ...
        if adjustM is None:
            result = state.simpleSum
        else:
            result = NP(state.simpleSum * adjustM)
        return result

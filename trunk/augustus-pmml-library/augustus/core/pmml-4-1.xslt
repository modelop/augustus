...
<!-- 
     ...
     Note: this stylesheet is a placeholder for future validation by XSLT.

     I (Jim Pivarski) do not think that the stylesheet is currently valid or complete.  I got it from

         http://www.dmg.org/current/pmml.xslt

     using a link on http://cs.pervasive.com/blogs/datarush/archive/2010/02/17/pmml-validation.aspx .

     It can be invoked by calling

         from lxml.etree import etree
         pmml = modelLoader(...)
         print etree.tostring(modelLoader.validateXslt(pmml))

     but the current stylesheet always complains that the <PMML> element is missing on documents that
     clearly have this element.  When it is working, it will become a part of PmmlModel.loadXml().

...
static void *thread() {
  ...
  gtk_window_set_title(GTK_WINDOW(window), "SVG Viewer");
...
static PyObject *svgviewer_view(PyObject *self, PyObject *args) {
  char *str_source = NULL;
  int str_size;
  if (!PyArg_ParseTuple(args, "es#", "utf-16", &str_source, &str_size)) {
    PyErr_SetString(PyExc_TypeError, "Source must be a string or unicode containing the SVG to draw.");
    return NULL;
  }
...
static PyMethodDef svgviewer_methods[] = {
  {"view", ((PyCFunction)(svgviewer_view)), METH_VARARGS, ""},
  {NULL}
};

PyMODINIT_FUNC initsvgviewer() {
  Py_InitModule3("svgviewer", svgviewer_methods, "");
  rsvg_init();
  g_thread_init(NULL);
  gdk_threads_init();
}

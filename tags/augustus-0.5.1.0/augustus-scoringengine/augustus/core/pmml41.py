...
from augustus.core.defs import Atom, IMMATURE, INVALID, MISSING, UNKNOWN, InvalidDataError, NameSpace
...
class DateTimeType:
    ...
    def __int__(self):
        return int(round((self.t - datetime.datetime(1970, 1, 1, 0, 0, 0, 0, self.t.tzinfo)).total_seconds()))
...
class ModelVerification(PMML):
    ...
    def get(self, field):
        return self.dataContext.cast.get(field, lambda x: x)(self.table.get(self.column[field]))
...
class SimpleRule(PMML):
    ...
    def calculateProbabilities(self):
        ...
        if sumOfProbabilities != 1.:
            raise XMLValidationError, "Probabilities of ScoreDistributions in SimpleRule do not add to 1.0 (are they a mixture of user-specified \"probability\" and probabilities calculated from \"recordCount\"?)"
...
class RegressionModel(PMML):
    ...
    def post_validate(self):
        ...
        raise NotImplementedError, "RegressionModel has not been implemented yet"
        ...
        if self.functionName == self.REGRESSION:
            ...
            if normalizationMethod == "none": self.prob =      lambda y: y
            elif normalizationMethod == "softmax": self.prob = lambda y: 1./(1. + exp(-y))
            elif normalizationMethod == "logit": self.prob =   lambda y: 1./(1. + exp(-y))
            elif normalizationMethod == "exp": self.prob =     lambda y: exp(y)
            else:
...
class RegressionTable(PMML):
    ...
    def post_validate(self):
        ...
        for numeric in self.matches(NumericPredictor):
        ...
        self.numericExponents = numpy.array(self.numericExponents)
        
        self.categoryFields = []
        self.categoryValues = []
        self.categoryCoefficients = []
        for category in self.matches(CategoricalPredictor):
    ...
    def evaluate(self, get):
        ...
        try:
            numericX = numpy.array(map(float, numericX))
        except (ValueError, AttributeError):
            raise InvalidDataError

        categoryX = [get(field) for field in self.categoryFields]
        if INVALID in categoryX: raise InvalidDataError
        categoryX = numpy.array([1. if x == value else 0. for x, value in zip(categoryX, self.categoryValues)])

        output = self.attrib["intercept"]
        try:
            output += sum((numericX**self.numericExponents) * self.numericCoefficients)
            output += sum(categoryX * self.categoryCoefficients)
        except FloatingPointError:
            raise InvalidDataError

        for predictorTerm in self.predictorTerms:
            y = predictorTerm.evaluate(get)
...
class PredictorTerm(PMML):
    ...
    def evaluate(self, get):
        ...
        return self.attrib["coefficient"] * reduce(operator.mul, values, 1.)
...
class NaiveBayesModel(PMML):
    ...
    def post_validate(self):
        self.isScorable = self.attrib.get("isScorable", True)
        raise NotImplementedError, "NaiveBaysesModel has not been implemented yet"
...
class TransformationDictionary(PMML):
    ...
    builtin = {
        ...
        "dateSecondsSinceYear": lambda t, year: (t.t - datetime.date(year, 1, 1)).days * 24*60*60 if isinstance(t, DateType)
                                                else int(round((t.t - datetime.datetime(year, 1, 1, 0, 0, 0, 0, t.t.tzinfo)).total_seconds())),
    ...
    def post_validate(self):
        ...
        for derivedField in self.matches(DerivedField):
            ...
            self.derivedFields[derivedField.attrib["name"]] = derivedField
            self.cast[derivedField.attrib["name"]] = castFunction(derivedField.attrib["optype"], derivedField.attrib["dataType"], [], [], False)
            self.optype[derivedField.attrib["name"]] = derivedField.attrib["optype"]
            self.dataType[derivedField.attrib["name"]] = derivedField.attrib["dataType"]
        ...
        for index, row in enumerate(self.table.rows):
            for column in columnsToCheck:
                try:
                    ...
                    raise RuntimeError, "Column \"%s\" missing from row %d of MapValues table%s" % (column, index, self.fileAndLine("<PMML>"))
                ...
                try:
                    ...
                    raise RuntimeError, "Column \"%s\" in row %d of MapValues table%s has no text" % (column, index, self.fileAndLine("<PMML>"))
...
class DataDictionary(PMML):
    ...
    def post_validate(self):
        ...
        for dataField in self.matches(DataField):
            self.dataFields[dataField.attrib["name"]] = dataField
    ...
    def __setitem__(self, index, value):
        if len(index) == 2 and isinstance(index[0], (int, long)) and isinstance(index[1], (int, long)):
            ...
            if self.sparse:
                matcell = self.lookup.get((i, j), None)
                if matcell is None:
                    matcell = new.instance(MatCell)
                    matcell.tag = "MatCell"
                    matcell.children = []
                    matcell.attrib = {"row": i + 1, "col": j + 1}
...
class MiningSchema(PMML):
    ...
    def post_validate(self):
        ...
        for miningField in self.matches(MiningField):
            self.miningFields[miningField.attrib["name"]] = miningField
...
class Node(PMML):
    ...
    def post_validate(self):
        if "score" in self.attrib:
            ...
            best = None
            self.score = None
            for i in self.scoreDistribution:
                probability = self.scoreDistribution[i.attrib["value"]]
                if best is None or probability > best:
                    best = probability
                    self.score = i.attrib["value"]
...
class X_ODG_OutputField(X_ODG_PMML, OutputField):
    xsd = load_xsdElement(X_ODG_PMML, """
  <xs:element name="X-ODG-OutputField">
    <xs:complexType>
      <xs:sequence>
        <xs:element maxOccurs="unbounded" minOccurs="0" ref="Extension" />
      </xs:sequence>
...
class X_ODG_PredictorTerm(X_ODG_PMML, PredictorTerm):
  ...
  """)
    ...
    def evaluate(self, get):
        if self.expression is None:
            return PredictorTerm.evaluate(self, get)
        ...
        return self.attrib["coefficient"] * output

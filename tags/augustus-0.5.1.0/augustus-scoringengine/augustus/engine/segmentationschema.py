...
class SegmentationScheme:
    ...
    def _convertFieldTypes(self, definition, field, typeconvert):
        ...
        try:
            ...
            raise ConfigurationError, " ".join([
                "Could not convert enumerated item with field=%s in configuration file" % field,
                "segment definition to the appropriate data type as defined in the pmml."])
        try:
            ...
            raise ConfigurationError, " ".join([
                "Could not convert partitioned item with field=%s in configuration file" % field,
                "segment definition to the appropriate data type as defined in the pmml."])
    ...
    def _createPMMLSegment(self, matchList):
        ...
        if len(predicateList) == 0:
            raise Exception, " ".join([
                "Encountered a list of zero predicates in SegmentationScheme's _createPMMLSegment; this should not",
                "ever be possible when using a validated config file."])
    ...
    def _setSegmentTemplate(self):
        ...
        if miningModel is None:
            raise ConfigurationError, " ".join([
                "The top level PMML model must be \"MiningModel\"",
                "if segments are to be generated."])
        ...
        if self._template:
            ...
            raise ConfigurationError, " ".join([
                "Must have a segment template defined in the PMML file in",
                "order to produce segment...the segment must be in the",
                "MiningModel and must have the id=\"ODG-SegmentTemplate\"",
                "as its id and have <False /> as the",
                "selection predicate."])

        if not self._template.exists(pmml.pmmlFalse):
            raise ConfigurationError, " ".join([
                "The selection predicate must be <False /> for the",
                "default segment defined in the PMML model."])

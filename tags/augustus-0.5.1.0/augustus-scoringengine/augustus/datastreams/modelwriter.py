...
class ModelWriterThread(threading.Thread):
    def run(self):
        if self.pickle:
            ...
            self.pmmlObject.write(
                fileName=self.fileName, indent=self.indent, linesep=self.linesep)
...
def getModelWriter(configOptions):
    ...
    if "outputFilename" not in configOptions.attrib: return None

    filename = configOptions["outputFilename"]
    ...
    return ModelWriter(
        filename, serialization=serialization, pickle=pickle, indent="    ", linesep=os.linesep)

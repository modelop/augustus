...
class pmmlErrorStrings:
  ...
  #used in scoring
  fieldMissing = "The field attribute is missing:\n"
  alternateMissing = "The alternate distribution is missing:\n"
  functionUndefined = "The function given is not defined:\n"
...
class choiceVI(pmmlChoice):
  """"""
  __types = [pmmlValue, pmmlInterval]
  __maximums = [None, None]
  def __init__(self, instances):
    """"""
    minimums = [None, None]
    pmmlChoice.__init__(self, choiceVI.__types, minimums, choiceVI.__maximums, instances)
  
  @staticmethod  
  def formatChildren(children):
    """"""
    return pmmlChoice.formatChildren(choiceVI.__types, choiceVI.__maximums, children)
...
class choiceTI(pmmlChoice):
  """"""
  __types = [pmmlTableLocator, pmmlInlineTable]
  __maximums = [1, 1]
  def __init__(self, instances):
    """"""
    minimums = [1, 1]
    pmmlChoice.__init__(self, choiceTI.__types, minimums, choiceTI.__maximums, instances)
  
  @staticmethod    
  def formatChildren(children):
    """"""
    return pmmlChoice.formatChildren(choiceTI.__types, choiceTI.__maximums, children)
...
class pmmlConstant(pmmlElement):
  ...
  def code(self, name, indent=""):
    ...
    if dataType:
      return indent + name + " = pmmlString.convert('" + value + "', '" + dataType + "')\n"
    else:
      try:
        if value.find(".") != -1:
          float(value)
          return indent + name +" = float(" + value + ")\n"
        elif value == str(int(value)):
          return indent + name + " = int(" + value + ")\n"
        elif value == str(long(value)):
          return indent + name + " = long(" + value + ")\n"
      except:
        pass
...
class choiceCF(pmmlChoice):
  ...
  __types = [pmmlConstant, pmmlFieldRef, pmmlNormContinuous, pmmlNormDiscrete, pmmlDiscretize, pmmlMapValues, pmmlAggregate]
  
  @staticmethod
  def add(aType):
    """Adds a type to the choiceCF.__types list.  Should only be used for the pmmlApply selfreferential loop."""
    choiceCF.__types.append(aType)
  ...
  def __init__(self, instances):
    ...
    pmmlChoice.__init__(self, choiceCF.__types, minimums, choiceCF.__maximums, instances)
    
  @staticmethod
  def formatChildren(children):
    """"""
    return pmmlChoice.formatChildren(choiceCF.__types, choiceCF.__maximums, children)
...
pmmlStandardFunctions = {
  ...
  "dateDaysSinceYear":lambda aList:(aList[0]-DT.datetime(aList[1],1,1)).days + 1,
  #"dateSecondsSinceYear":lambda aList:(aList[0]-DT.datetime(aList[1],1,1)).days * 86400 + (aList[0]-DT.datetime(aList[1],1,1)).seconds,

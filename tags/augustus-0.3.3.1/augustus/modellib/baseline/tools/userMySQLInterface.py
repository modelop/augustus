...
def insertEv(cv,irec,e,evtable,log=sys.stdout):
    ...
    if (identifier is None): 
        msg="\n\tCannot insert event of unknown origin. "
        msg+="\n\tinto MySQLdb with  insertEv function."
        log.warning(msg)
    else:
        try:
            sql='SELECT CURDATE()'
            ...
            msg="\n\tUnable to insert event into MySqldb "
            msg+="\n\t with insertEv function."
            log.warning(msg)
    return
...
def configsToFiles(dbname,username,pwd,subdir):
    ...
    for row in cv:
        ...
        if not m:        
            outfile.write ("# FROM configs TABLE IN DATABASE %s - DO NOT EDIT\n" % (dbname))

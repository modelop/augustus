...
if __name__ == "__main__":
  ...
  for _e in scoredEvents:
    ...
    if len(segment) != 0:
      ...
      # batched processing with no segments->only one score.
      scores[None] = sc
  ...
  for segSchema in segments.keys():
    if len(segSchema) == 0:
      log.info("INFO: No evident segmentation scheme. Assume unsegmented scoring\n")

      event = ET.SubElement(events,'Event')
      event.attrib['score'] = scores[None]

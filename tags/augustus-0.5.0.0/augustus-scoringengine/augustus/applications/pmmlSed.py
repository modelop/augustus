...
class Global:
    ...
    def __repr__(self):
        return "<Global %s>" % self.d

    def __getattr__(self, name):
        try:
            return self.d[name]
        except KeyError:
            raise RuntimeError, "No global variable named \"%s\"" % name
    ...
    def __delattr__(self, name):
        try:
            ...
            raise RuntimeError, "No global variable named \"%s\", cannot delete" % name
  ...
  """)
    ...
    def post_validate(self):
        if "as" not in self.attrib:
            self["as"] = self["library"]

        try:
            exec("import %s as tmp" % self["library"])
        except ImportError as err:
            raise XMLValidationError, "Context element could not load library \"%s\"" % self["library"]

        if self["as"] == "*":
            self.context = tmp.__dict__
        else:
            self.context = {self["as"]: tmp}
...
class PythonFunction(PmmlSed):
  ...
  """)
    ...
    def post_validate(self):
        ...
        try:
            exec "".join(cdatas[0].text) in context

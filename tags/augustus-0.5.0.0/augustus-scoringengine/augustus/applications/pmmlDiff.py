...
def pmmlDiff(name1, name2, validate=False, numSigfigs=6, header=False, extensions=False):
    ...
    if extensions:
        ...
        index1 = [i for i, x in file1.walk(lambda x: isinstance(x, pmml.Extension))]; index1.insert(0, None)
        index2 = [i for i, x in file2.walk(lambda x: isinstance(x, pmml.Extension))]; index2.insert(0, None)

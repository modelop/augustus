...
class XMLStreamLoader(xml.sax.handler.ContentHandler):
    ...
    def getRowFrom(self, string):
        ...
        try:
            ...
            print "Bad data encountered in stream: %s" % str(err)
            sys.exit(-1)
    ...
    def endElement(self, name):
        ...
        while name != currentName:
            ...
            if len(self._stack) > 1:
                ...
                print "Error reading the xml..failed in element", name
                sys.exit(-1)
...
def compare_elements(elementA, elementB, sigfigs):
    ...
    if tagA != tagB:
        ...
        for childA, childB in izip(childrenA, childrenB):
            if isinstance(childA, tuple) and isinstance(childB, tuple):
                matched = compare_elements(childA, childB, sigfigs)
                if not matched:
                ...
                if childA != childB:
                    try:
                        ...
                        return False
                    percent_diff = abs(childA - childB)/(sum([childA, childB])/2)
                    if percent_diff > pow(0.1, sigfigs):
                        print "Content differs:", childA, "within", tagA, "(first file)",
                        print "and", childB, "within", tagB, "(second file)",
                        return False
            elif childA == childB:
...
def compare_lines(lineA, lineB, sigfigs):
    ...
    return compare_elements(rowA, rowB, sigfigs)


def compare_files(fileNameA, fileNameB, sigfigs, maxlines):

    with open(fileNameA) as fileA:
        with open(fileNameB) as fileB:
            lines = 0
            for lineA, lineB in izip(fileA, fileB):
                success = compare_lines(lineA, lineB, sigfigs)
                if not success:
                    print "at line", lines
                    return False

            ...
            while True:
                try:
                    ...
                    for name, resultValue in results:
                        ...
                        if not verified:
                            errorString = "Verification failure in ModelVerification %d%s in model %s%s, row %d (\"<%s>\"): %s (\"%s\") expected to be %s, calculated to be %s" % (verificationBlockCounter, verification.fileAndLine(), model.tag, model.fileAndLine(), verification.table.index, verification.table.rows[verification.table.index].content(), name, verification.column[name], str(expectedValue), str(resultValue))
                    ...
                    if reportInScores and outputWriter:
                        outputWriter.write(verificationErrorsBlock, eventTags=[("model", "%s on line %s" % (model.tag, str(getattr(model, "lineStart", "unknown")))), ("block", str(verificationBlockCounter))], eventName="ModelVerificationFailures")

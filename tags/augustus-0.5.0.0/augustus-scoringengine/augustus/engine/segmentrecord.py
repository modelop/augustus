...
class SegmentRecord:
    ...
    def initialize(self, consumerParameters, producerParameters, existingSegment=False):
        """Initialize the consumer, the producer, and start the
        maturity count.

        consumerParameters and producerParameters are dictionaries
        that are passed directly to the algorithms.  Only
        producerParameters has been used, yet (consumer is fully
        specified by PMML)."""
        ...
        if not existingSegment:
            ...
            if "resume" in producerParameters and producerParameters["resume"] is True:
                self.updator.initialize({COUNT: self.pmmlModelMaturity.attrib["numUpdates"]})
            self.lock = self.pmmlModelMaturity.attrib["locked"]

        self.consumerAlgorithm.initialize(**consumerParameters)
        self.producerAlgorithm.initialize(**producerParameters)

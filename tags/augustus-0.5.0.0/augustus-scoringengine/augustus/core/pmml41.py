...
from augustus.core.extramath import MAXFLOAT, sqrt, exp, log, floor, pi, erf, gammq, gammln
...
class Metric:
    ...
    def configure(self, fieldWeights, missingValueWeights, compareFunctions, similarityScales, comparisonTables):
        ...
        if self.kind is self.DISTANCE:
            if self.missingValueWeights is None:
                self.metricMissing = lambda x, y: MISSING

        elif self.kind is self.SIMILARITY:
            self.metricMissing = lambda x, y: MISSING
...
class euclidean(PMML, squaredEuclidean):
    ...
    def metric(self, data, cluster):
        return math.sqrt(squaredEuclidean.metric(self, data, cluster))

    def metricMissing(self, data, cluster):
        return math.sqrt(squaredEuclidean.metricMissing(self, data, cluster))
...
class RegressionModel(PMML):
    ...
    def top_validate(self, dataContext):
        if "targetFieldName" in self.attrib and self.attrib["targetFieldName"] not in dataContext.cast:
            raise XMLValidationError, "%s references field \"%s\" but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["targetFieldName"], dataContext.contextString())
...
class OutputField(PMML):
    ...
    def top_validate(self, dataContext):
        ...
        if "targetField" in self.attrib:
            if self.attrib["targetField"] not in dataContext.cast:
                raise XMLValidationError, "%s references field \"%s\" but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["targetField"], dataContext.contextString())

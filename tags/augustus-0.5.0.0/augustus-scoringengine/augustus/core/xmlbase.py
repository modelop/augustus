...
class XML:
    ...
    def _treeindex_descend(self, obj, treeindex):
        if isinstance(treeindex, (list, tuple)):
            # TANYA::trying to speed up
            obj = reduce(lambda subitem, i: subitem.children[i], treeindex[:-1], obj)
    ...
    class _DepthIterator:
        ...
        def _children_iterators(self):
            if hasattr(self.current, "children") and len(self.current.children):
                ...
                return chain_from_iterable(itertools.imap(childiterator, self.current.children, itertools.count()))
            return []
        ...
        def _attrib_iterators(self):
            if hasattr(self.current, "attrib") and len(self.current.attrib):
                ...
                return chain_from_iterable(itertools.imap(attribiterator, items))
            return []
    ...
    def descendant(self, test=None, which=0, maxdepth=None, exception=True):
        ...
        for i, elem in self.walk(None, maxdepth, attrib=False, special=False):
            if test is None:
                ...
                raise TypeError, "The 'test' must be a boolean, a class, a boolean-returning function, or a tag name string (XML tree of %s)" % self

        if exception:
            raise ValueError, "The 'test' was not satisfied in XML tree of %s" % self
        else:
            return None
    ...
    def _xml(self, indent="    ", linesep=os.linesep, spaces=""):
        ...
        if len(children) == 0:
            return ["%s<%s%s />" % (spaces, self.tag, attrib)]

        elif len(children) == 1 and isinstance(children[0], XMLText):
            return ["%s<%s%s>%s</%s>" % (spaces, self.tag, attrib, escape(linesep.join(children[0]._xml(indent, linesep, spaces))), self.tag)]
    ...
    def write(self, fileName, indent="    ", linesep=os.linesep, encoding="utf-8"):
        ...
        output = self._xml(indent, linesep, "")
        ...
        if os.linesep is not None and os.linesep != "":
            for line in output:
                f.write(line)
                f.write(os.linesep)
        else:
...
class XSDElementRef(XSDMinMax):
    ...
    def incrementIndex(self, xmlChildren, index):
        matches = 0
        while index < len(xmlChildren) and (xmlChildren[index].tag == self.name or isinstance(xmlChildren[index], self.classMap[self.name])):
            matches += 1
            index += 1
    ...
    def incrementIndex(self, xmlChildren, index):
        ...
        if self.minOccurs is not None:
            if matches < self.minOccurs and (complaints or self.somethingNeeded()):
                raise XMLValidationError, "Too few \"%s\" sequences (%d observed, minimum is %d)" % (self.name, matches, self.minOccurs)
    ...
    def incrementIndex(self, xmlChildren, index):
        ...
        if self.minOccurs is not None:
            if matches < self.minOccurs and self.somethingNeeded():
                raise XMLValidationError, "Too few \"%s\" choice blocks (%d observed, minimum is %d)" % (self.name, matches, self.minOccurs)
...
class XSDList(XSDBuiltinTypeCheck):
    ...
    def validateText(self, text):
        return map(self.validateType, [x.lstrip("\"").rstrip("\"") for x in re.findall(self.findall, text)])
    ...
    def validate(self, xml):
        ...
        if self.justCheckNumber is False:
            ...
            if index != len(xmlChildren):
                ...
                raise XMLValidationError, "Too many subelements: %s (\"%s\") was unexpected" % (cardinal, xmlChildren[index].tag)
...
def load(stream, base=None, validation=True, dropSpecial=False, lineNumbers=False):
    ...
    class ContentHandler(xml.sax.handler.ContentHandler):
        ...
        def startElement(self, tag, attrib):
            ### Is this still needed?  If not, let's get rid of it
            # if "tag" in attrib:
            #     attrib2 = {}
            ...
            #             attrib2[n] = v
            #     attrib = attrib2
            ### End cause-a-crash-if-it-turns-out-we-really-do-need-it

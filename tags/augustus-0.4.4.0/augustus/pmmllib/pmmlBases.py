...
def makeSecondsSince(when, year):
  """returns an integer which is the number of seconds
  from January 1 of year to when"""
  return int( int(when) - time.mktime((year,1,1,0,0,0,0,1,-1)) )
  ...
  def convert(value, dataType):
    ...
    if dataType == "integer":
      try:
        ...
        #If value is an empty string treat it as a missing value otherwise raise an error
        if value == '':
          return None
        else:
          raise
      ...
      if isinstance(value, int):
        #Assume that we're trying to convert a dateTimeSecondsSince[1970]
        #TODO: Handle other data types that we store as integers.
        try:
        ...
        # input is a string in iso 8601 format yyyy-mm-dd
        # Actually, PMML specifies the www.w3.org variation on 8601.
        # This means we don't support right-truncated formats or the
        # format without hyphens
        if value[0]=='-':
            # Negative years *are* permitted by the standard but
            # not supported by us at this time. The Datetime module
        ...
        if len(value)!=10:
          ...
          raise
        try:
          return DT.date(*[int(c) for c in value.split('-')])
        except:
          print 'Could not convert %s\n'%value
          raise
    elif dataType == "time":
...
class pmmlContainer:
  ...
  def validateInstance(self, instance):
    ...
    if not isinstance(instance, self.__types):
      for aType in self.__types:
        if issubclass(aType, pmmlChoice):
          for choiceType in aType.types:
            if isinstance(instance, choiceType):
              return True
      raise pmmlError, "Given object type (%s) not allowed in this container." % (str(instance))
...
class pmmlList(pmmlContainer):
  ...
  def removeInstance(self, instance):
    ...
    if instance in self.__list:
      ...
      return
    #Handle the case of removing instances that inherit from pmmlChoice
    for item in self.__list:
      if isinstance(item, pmmlChoice):
        if instance in item.getChildren():
...
class pmmlSequence(pmmlList):
  ...
  def addInstance(self, child):
    ...
    for aType in self.__setTypes:
      if issubclass(aType, pmmlChoice):
        for choiceType in aType.types:
          if isinstance(child, choiceType):
  ...
  def replaceInstance(self, oldChild, newChild):
    ...
    index = 0
    for aType in self.__setTypes:
      #This won't work if you want to replace a child of a pmmlChoice group
...
class pmmlElement:
  ...
  def __init__(self, name, child, attributeNames=[], attributes={}, requiredAttributes=[], whiteSpaceImportant=False):
    ...
    #If all of the attributes didn't used we must have an extra one.
    if count != len(attributes):
      for att_name in attributes:
        if att_name not in attributeNames:
          #TODO: This obviously needs to be changed to a logger
          print "WARNING: %s is not a valid attribute of the %s element." % (att_name, self.__name)
...
def pmmlElementByXSD(xsd, glob, loc):
  xsd = re.sub("xs:", "", xsd)
  xsd = ElementTree.fromstring(xsd)
  name = xsd.attrib["name"]
  model = (re.search("Model$", name) is not None)
  xsd = xsd.find("complexType")
  if xsd is None:
    raise NotImplementedError, "Internal programming error: please check the implementation of pmml%s" % name

  def getall(node, defaultmin=1, defaultmax=1):
    if node.tag in ("element", "group"):
      thetype = node.attrib["ref"]
      if thetype == "NUM-ARRAY": thetype = "Array"

      maximum = node.attrib.get("maxOccurs", defaultmax)
      minimum = node.attrib.get("minOccurs", defaultmin)
      if maximum == "unbounded": maximum = None
      else: maximum = int(maximum)
      if minimum == "0": minimum = None
      else: minimum = int(minimum)

      types.append("pmml%s" % thetype)
  ...
  maximums = []
  minimums = []
  sequence = xsd.find("sequence")
  getall(sequence)
  types = ", ".join(types)
  maximums = ", ".join(map(str, maximums))
  minimums = ", ".join(map(str, minimums))

  attributeNames = []
  requiredAttributes = []
  for child in xsd.getchildren():
    if child.tag == "attribute":
      thename = child.attrib["name"]
      required = child.get("use", "optional")

      attributeNames.append(thename)
      if required == "required":
        requiredAttributes.append(thename)

  attributeNames = ", ".join(map(lambda x: '"%s"' % x, attributeNames))
  requiredAttributes = ", ".join(map(lambda x: '"%s"' % x, requiredAttributes))

  if model:
    modeltext1 = ", genericModel"
    modeltext2 = "genericModel.__init__(self)"
  else:
    modeltext1, modeltext2 = "", ""

  exec r"""
class pmml%(name)s(pmmlElement%(modeltext1)s):
  def __init__(self, name="", attributes={}, children=[]):
    if getattr(self, "_pre_init_hook", None) is not None:
      name, attributes, children = self._pre_init_hook(name, attributes, children)
    types = [%(types)s]
    maximums = [%(maximums)s]
    minimums = [%(minimums)s]
    (extras, children) = pmmlSequence.formatChildren(types, maximums, children)
    if extras != []:
      raise pmmlError, "%(name)s:  " + pmmlErrorStrings.elements
    myChild = pmmlSequence(types, minimums, maximums, children)
    attributeNames = [%(attributeNames)s]
    requiredAttributes = [%(requiredAttributes)s]
    %(modeltext2)s
    pmmlElement.__init__(self, "%(name)s", myChild, attributeNames, attributes, requiredAttributes)
    if getattr(self, "_post_init_hook", None) is not None:
      self._post_init_hook(name, attributes, children)
""" % vars() in glob, loc

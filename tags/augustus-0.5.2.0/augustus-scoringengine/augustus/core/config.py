  ...
  """)
    ...
    def post_validate(self):
        if self.attrib["stage"] == "segment" and "segment" not in self.attrib:
            raise XMLValidationError, "If stage == 'segment', a segment name must be provided"

        if self.attrib["stage"] != "segment" and "segment" in self.attrib:
            raise XMLValidationError, "If stage != 'segment', a segment name must not be provided"
...
""")
    ...
    def post_validate(self):
        if self.attrib["model"] not in self.available.keys():
            raise xmlbase.XMLValidationError, "Unrecognized model \"%s\" (%s are available)" % (self.attrib["model"], self.available.keys())

        if self.attrib["algorithm"] not in self.available[self.attrib["model"]]:
            raise xmlbase.XMLValidationError, "Unrecognized algorithm \"%s\" for model %s (%s are available)" % (self.attrib["algorithm"], self.attrib["model"], self.available[self.attrib["model"]])
...
""")
    ...
    def post_validate(self):
        try:
            ...
            raise xmlbase.XMLValidationError, "External segmentation schema file \"%s\" could not be opened: %s" % (self.attrib["fileName"], str(err))
...
""")
    ...
    def post_validate(self):
        ...
        if len(cdatas) != 1:
            raise XMLValidationError, "A CustomProcessing object must contain exactly one CDATA"
    ...
    def doEvent(self, syncNumber, eventNumber, get, matchingSegments):
        ...
        for segment in matchingSegments:
            ...
            segment.score = lambda: segment.consumer.score(syncNumber, segment.get)
            segment.update = lambda: segment.producer.update(syncNumber, segment.get)
...
class PersistentStorage(Config):
  ...
  """)
    ...
    def post_validate(self):
        ...
        if m is None:
            raise XMLValidationError, "Connect string must be PROTOCOL://ADDRESS, not \"%s\"" % self.attrib["connect"]
        self.protocol, self.address = m.groups()
        self.protocol = self.protocol.lower()

        if self.protocol not in self.protocols:
            raise XMLValidationError, "Protocol \"%s\" not recognized; must be one of %s" % (self.protocol, self.protocols)
    ...
    def write(self):
        ...
        if self.protocol == "json":
            ...
            json.dump({"Global": dbglobal, "Segments": dbsegments}, file(self.address, "w"), cls=JSONEncoderWithNameSpace)

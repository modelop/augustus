...
def newInstance(tag, attrib={}, children=[], base=PMML):
    output = new.instance(base.classMap[tag])
    output.tag = tag
...
def castFunction(optype, dataType, pmmlIntervals, pmmlValues, isCyclic):
    ...
    # a special case: continuous with values
        ...
        for val in pmmlValues:
            try:
                ...
                raise XMLValidationError, "Could not cast Value's \"%s\" as %s: %s" % (val.attrib["value"], dataType, str(err))
    ...
    # a special case: cyclic ordinals
        ...
        if len(pmmlValues) != 2:
            raise XMLValidationError, "Cyclic ordinal types must have two Values (first and last in the repeating sequence)"

        try:
            int(pmmlValues[0].attrib["value"])
            int(pmmlValues[1].attrib["value"])
        except ValueError:
            raise XMLValidationError, "Values for cyclic ordinal type must be integers"

        class CyclicOrdinalType:
    ...
    # a special case: cyclic continuous
        ...
        if len(pmmlIntervals) != 1 or "leftMargin" not in pmmlIntervals[0].attrib or "rightMargin" not in pmmlIntervals[0].attrib:
            raise XMLValidationError, "Cyclic continuous types must have one finite Interval"

        class CyclicContinuousType:
...
def treatmentFunction(miningField, cast):
    ...
    if "missingValueReplacement" in miningField.attrib:
        try:
            ...
            raise XMLValidationError, "MiningField missingValueReplacement \"%s\" cannot be cast to the appropriate type: %s" % (miningField.attrib["missingValueReplacement"], str(err))
    else:
...
class DataContext:
    def __init__(self, parent, dataDictionary, transformationDictionary, miningSchema, localTransformations, functionName):
        ...
        if self.transformationDictionary is not None:
            ...
            if len(conflict) > 0:
                raise XMLValidationError, "MiningSchema field names and TransformationDictionary field names conflict: %s" % repr(conflict)
        ...
        if self.localTransformations is not None:
            ...
            if len(conflict) > 0:
                raise XMLValidationError, "MiningSchema field names and LocalTransformations field names conflict: %s" % repr(conflict)
        ...
        if self.transformationDictionary is not None and self.localTransformations is not None:
            ...
            if len(conflict) > 0:
                raise XMLValidationError, "TransformationDictionary field names and LocalTransformations field names conflict: %s" % repr(conflict)
            ...
            for derivedField in self.transformationDictionary.matches(DerivedField):
                for fieldRef in derivedField.matches(FieldRef, maxdepth=None):
                    if fieldRef.attrib["field"] not in context:
                        raise XMLValidationError, "FieldRef \"%s\" in the TransformationDictionary (DerivedField %s) is not accessible from one of the MiningSchema contexts (%s)" % (fieldRef.attrib["field"], derivedField.attrib["name"], ", ".join(context))
...
class root(PMML):
    ...
    def post_validate(self):
        ...
        if self.attrib["version"] != "4.1":
            raise XMLValidationError, "PMML version in this file is \"%s\" but this is a PMML 4.1 interpreter" % self.attrib["version"]
...
class TableLocator(PMML):
    ...
    def post_validate(self):
        ...
        try:
            ...
            raise XMLValidationError, "TableLocator xmlfile Extension points to a non-existent file \"%s\"" % extensions[0].attrib["value"]
        except xmlbase.XMLError, err:
            raise XMLValidationError, "TableLocator's XML file \"%s\" is not correctly formatted (%s)" % (extensions[0].attrib["value"], str(err))
        ...
        if self.exists(ModelVerification):
            for segmentation in self.matches(Segmentation):
                if segmentation.attrib["multipleModelMethod"] != "selectFirst":
                    raise XMLValidationError, "ModelVerification can only be used with selectFirst segmentation"
...
class ResultField(PMML):
    ...
    def top_validate(self, dataContext):
        if self.attrib["name"] not in dataContext.cast:
            raise XMLValidationError, "%s references field \"%s\" but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["name"], dataContext.contextString())
...
class Regression(PMML):
    ...
    def post_validate(self):
        raise NotImplementedError, "Regression has not been implemented because embedded models will be removed in PMML 4.1 in favor of model-chaining"
...
class DecisionTree(PMML):
    ...
    def post_validate(self):
        raise NotImplementedError, "DecisionTree has not been implemented because embedded models will be removed in PMML 4.1 in favor of model-chaining"
...
class Scorecard(PMML):
    ...
    <xs:element name="Scorecard">
        <xs:complexType>
            ...
            <xs:attribute default="pointsBelow" name="reasonCodeAlgorithm" type="xs:string">
                <xs:simpleType>
                    <xs:restriction base="xs:string">
            ...
            <xs:attribute default="other" name="baselineMethod" type="xs:string">
                <xs:simpleType>
                    <xs:restriction base="xs:string">
                        <xs:enumeration value="max" />
    ...
    def top_validate_output(self, dataContext, pmmlOutput):
        ...
        if self.verificationRefersToOutput:
            ...
            if len(self.predicted) != 1:
                raise XMLValidationError, "If no Output is present or no VerificationFields refer to an OutputField, there must be only one predicted value"
        ...
        for field in self.column.keys():
            if field not in self.dataContext.cast and field not in self.predicted:
                raise XMLValidationError, "VerificationField \"%s\" is neither in the current data context nor is it a predicted field in the MiningSchema" % field
...
class RuleSetModel(PMML):
    ...
    def post_validate(self):
        ...
        if self.attrib["functionName"] != "classification":
            raise XMLValidationError, "The only valid RuleSetModel functionNames are: 'classification'"
        ...
        for rule in simpleRules:
            if "id" in rule.attrib:
                ...
                if name not in usedNames:
                    ...
                    raise XMLValidationError, "SimpleRule id=\"%s\" appears more than once" % name
...
class SimpleRule(PMML):
    ...
    def calculateProbabilities(self):
        ...
        if abs(sumOfProbabilities - 1.) > 1e-5:
            raise XMLValidationError, "Probabilities of ScoreDistributions in SimpleRule do not add to 1.0, they add to %g (are they a mixture of user-specified \"probability\" and probabilities calculated from \"recordCount\"?)" % sumOfProbabilities
...
class ClusteringModel(PMML):
    ...
    def post_validate(self):
        ...
        if self.attrib["functionName"] != "clustering":
            raise XMLValidationError, "The only valid ClusteringModel functionNames are: 'clustering'"
        ...
        #     raise XMLValidationError, "Number of observed Clusters (%d) does not match numberOfClusters attribute (%d)" % (len(clusters), self.numberOfClusters)
        ...
        for i, cluster in enumerate(clusters):
            ...
            if "id" not in cluster.attrib:
                newId = "%d" % (i+1)
                if newId in usedIds:
                    raise XMLValidationError, "Assigning \"%s\" to a cluster without an id, but this id is already in use by another cluster" % newId
        ...
        if len(occurrences) != 0:
            raise XMLValidationError, "Multiple occurrences of some cluster ids: %s" % str(occurrences)
        ...
        if self.exists(MissingValueWeights):
            ...
            if len(missingValueWeights) != len(self.fields):
                raise XMLValidationError, "Length of MissingValueWeights (%d) does not match the number of ClusteringFields (%d)" % (len(missingValueWeights), len(self.fields))
        ...
        if self.child(ComparisonMeasure).attrib["kind"] == "distance":
            for clusteringField in self.matches(ClusteringField):
                ...
                if optype != "continuous":
                    raise XMLValidationError, "ClusteringModel has ComparisonMeasure.kind == 'distance' yet ClusteringField \"%s\" has optype \"%s\" in MiningSchema's active context %s" % (field, optype, dataContext.contextString())
...
class Cluster(PMML):
    ...
    def post_validate(self):
        if "size" in self.attrib:
            array = self.child(Array, exception=False)
            if array is not None:
                if len(array.values) != self.attrib["size"]:
                    raise XMLValidationError, "Cluster size attribute does not match its Array size"
    ...
    def post_validate(self):
        if "compareFunction" in self.attrib and self.attrib["compareFunction"] == "gaussSim":
            if "similarityScale" not in self.attrib:
                raise XMLValidationError, "ClusteringField with compareFunction == \"gaussSim\" requires a similarityScale"

    def top_validate(self, dataContext):
        if self.attrib["field"] not in dataContext.cast:
            raise XMLValidationError, "%s references field \"%s\" but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["field"], dataContext.contextString())
...
class ComparisonMeasure(PMML):
    ...
    def post_validate(self):
        ...
        if metric.kind != kind:
            raise XMLValidationError, "ComparisonMeasure kind (%s) does not match the metric's kind (%s, %s)" % (self.attrib["kind"], metric.__class__.__name__, str(metric.kind))
...
class euclidean(PMML, squaredEuclidean):
    xsd = load_xsdElement(PMML, """
...
class RegressionModel(PMML):
    ...
    def post_validate(self):
        ...
        if self.attrib["functionName"] == "classification":
            ...
            raise XMLValidationError, "The only valid TreeModel functionNames are: 'regression', 'classification'"
        ...
        if self.functionName == self.REGRESSION:
            if len(self.regressionTables) != 1:
                raise XMLValidationError, "RegressionModels with functionName='regression' must have exactly one RegressionTable, not %d" % len(self.regressionTables)
            ...
            if normalizationMethod == "none":
                ...
                raise XMLValidationError, "The only valid RegressionModel normalizationMethods with functionName='regression' are: 'none', 'softmax', 'logit' (same as 'softmax'), and 'exp'"

        else:
            if len(self.regressionTables) < 2:
                raise XMLValidationError, "RegressionModels with functionName='classification' must have at least two RegressionTables, not %d" % len(self.regressionTables)
            ...
            for regressionTable in self.regressionTables:
                ...
                if targetCategory is None:
                    raise XMLValidationError, "RegressionModels with functionName='classification' must have a targetCategory in each RegressionTable"
                if targetCategory in valuesSeen:
                    raise XMLValidationError, "RegressionModels with functionName='classification' are not allowed to have duplicate targetCategories: \"%s\" is seen more than once" % targetCategory
    ...
    def top_validate_model(self, dataContext):
        if "targetFieldName" in self.attrib:
            ...
            if miningField is None:
                raise XMLValidationError, "%s references \"%s\" with targetFieldName, but there is no predicted MiningField with this name" % (self.__class__.__name__, self.attrib["targetFieldName"])
        ...
        if self.functionName == self.CLASSIFICATION:
            ...
            if normalizationMethod == "none": self.prob =        lambda yall: yall
                ...
                elif normalizationMethod == "cloglog": F = lambda yall: 1. - numpy.exp(-numpy.exp(yall))
    ...
    def top_validate(self, dataContext):
        if "name" in self.attrib:
            if self.attrib["name"] in dataContext.cast:
                raise XMLValidationError, "The %s name parameter must be unique, but \"%s\" is found in the MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["name"], dataContext.contextString())

        for fieldRef in self.fieldRefs:
            if dataContext.optype[fieldRef["field"]] != "continuous":
                raise XMLValidationError, "FieldRefs in a PredictorTerm must all refer to continuous features; \"%s\" is %s" % (fieldRef["field"], dataContext.optype[fieldRef["field"]])
...
class OutputField(PMML):
    ...
    def post_validate(self):
        ...
        if "feature" in self.attrib:
            ...
            if self.feature is None:
                raise XMLValidationError, "Unrecognized OutputField feature \"%s\"" % self.attrib["feature"]
        ...
        if badAnywhere:
            raise XMLValidationError, " ".join([
                "An OutputField has chosen tag %s which contains at least one disallowed character." % self.displayName,
                "Disallowed: %s" % str(badAnywhere.group())])
        badInitial = re.match("[\-.0-9]", self.displayName)
        if badInitial:
            raise XMLValidationError, " ".join([
                "An OutputField has chosen tag %s, which begins with a disallowed initial character." % self.displayName,
                "Disallowed: %s" % str(badInitial.group())])
        ...
        if self.feature is self.transformedValue:
            if self.expression is None:
                raise XMLValidationError, "If an OutputField's feature is transformedValue, then it must contain an EXPRESSION"

        if self.feature is self.decision:
            if self.expression is None:
                raise XMLValidationError, "If an OutputField's feature is decision, then it must contain an EXPRESSION"

            if self.decisions is None:
                raise XMLValidationError, "If an OutputField's feature is decision, then it must contain a Decisions block"

    def top_validate(self, dataContext):
        if self.feature is None:
            if self.attrib["name"] not in dataContext.cast:
                raise XMLValidationError, "%s references \"%s\" (without a feature attribute) but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["name"], dataContext.contextString())
            ...
            if miningField is None:
                raise XMLValidationError, "%s references \"%s\" with feature %s, but there is no predicted MiningField with this name" % (self.__class__.__name__, self.attrib["name"], str(self.feature))
        ...
        if "targetField" in self.attrib:
            ...
            if miningField is None:
                raise XMLValidationError, "%s references \"%s\" with targetField, but there is no predicted MiningField with this name" % (self.__class__.__name__, self.attrib["targetField"])
    ...
    def makeVerbose(self, dataContext=None):
        if "optype" not in self.attrib or "dataType" not in self.attrib:
            ...
            elif feature in ("probability", "residual", "standardError", "clusterAffinity", "entityAffinity"):
                optype = "continuous"
                dataType = "double"
    ...
    def post_validate(self):
        ...
        for pairCounts in self.matches(PairCounts):
            ...
            if inputValue in self.pairCounts:
                raise XMLValidationError, "Multiple PairCounts are associated with value \"%s\"" % inputValue
        ...
        if self.derivedField is not None:
            ...
            if not isinstance(expression, Discretize):
                raise XMLValidationError, "In a BayesInput's DerivedField, the only allowed expression is Discretize, not \"%s\"" % expression.__class__.__name__

            if self.attrib["fieldName"] != expression.attrib["field"]:
                raise XMLValidationError, "The BayesInput fieldName \"%s\" is different from its Discretize field \"%s\"" % (self.attrib["fieldName"], expression.attrib["field"])

    def top_validate(self, dataContext):
        if self.attrib["fieldName"] not in dataContext.cast:
            raise XMLValidationError, "%s references field \"%s\" but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["fieldName"], dataContext.contextString())

        if dataContext.optype[self.attrib["fieldName"]] == "continuous":
            if self.derivedField is None:
                raise XMLValidationError, "A continuous feature (\"%s\") cannot be used in a BayesInput without discretization: supply a DerivedField" % self.attrib["fieldName"]
        ...
        for inputValue in self.pairCounts.keys():
            try:
                ...
                raise XMLValidationError, "Could not cast PairCounts value \"%s\" for BayesInput field \"%s\": %s" % (inputValue, self.attrib["fieldName"], str(err))
...
class BayesOutput(PMML):
    ...
    def top_validate(self, dataContext):
        ...
        if miningField is None:
            raise XMLValidationError, "%s references \"%s\" with fieldName, but there is no predicted MiningField with this name" % (self.__class__.__name__, self.attrib["fieldName"])
...
class TargetValueCount(PMML):
    ...
    def post_validate(self):
        if self.attrib["count"] < 0.:
            raise XMLValidationError, "The count must be non-negative, not %g" % self.attrib["count"]
...
class Anova(PMML):
    ...
    def top_validate(self, dataContext):
        if "target" in self.attrib and self.attrib["target"] not in dataContext.cast:
            raise XMLValidationError, "%s references field \"%s\" but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["target"], dataContext.contextString())
...
class TransformationDictionary(PMML):
    ...
    def post_validate(self):
        ...
        if len(conflict) > 0:
            raise XMLValidationError, "User-defined function names conflict with built-in functions: %s" % repr(conflict)
        ...
        for derivedField in self.matches(DerivedField):
            if "name" not in derivedField.attrib:
                raise XMLValidationError, "All DerivedFields in a LocalTransformations must have \"name\" attributes"

            name = derivedField.attrib["name"]
            if name in self.derivedFields:
                raise XMLValidationError, "DerivedField \"%s\" appears more than once" % name
...
class LocalTransformations(PMML):
    ...
    def post_validate(self):
        ...
        for derivedField in self.matches(DerivedField):
            ...
            name = derivedField.attrib["name"]
            if name in self.derivedFields:
                raise XMLValidationError, "DerivedField \"%s\" appears more than once in the same LocalTransformation" % name
            ...
            if "groupField" in aggregate.attrib:
                if self.groupField is None:
                    ...
                    if self.groupField != aggregate.attrib["groupField"]:
                        raise XMLValidationError, "DerivedField contains Aggregates with different groupFields (\"%s\" and \"%s\")" % (self.groupField, aggregate.attrib["groupField"])
        ...
        for fieldRef in self.matches(FieldRef, maxdepth=None):
            ...
            if derivedField is not None and derivedField.groupField is not None:
                ...
                if self.groupField != derivedField.groupField:
                    raise XMLValidationError, "DerivedField contains Aggregates with different groupFields (\"%s\" and \"%s\", through a \"%s\" reference)" % (self.groupField, derivedField.groupField, fieldRef.attrib["field"])
        ...
        if "dataType" in self.attrib:
            ...
            if self.attrib["dataType"] in pmmlBuiltinFloats:
                if numpy.isnan(self.value) or numpy.isinf(self.value):
                    raise XMLValidationError, "Floating-point constant should not be NaN/Inf"
    ...
    def top_validate_output(self, dataContext, pmmlOutput):
        if self.attrib["field"] not in dataContext.cast:
            if self.inOutput and pmmlOutput is not None:
                if self.attrib["field"] not in pmmlOutput.featuredFields:
                    raise XMLValidationError, "%s references field \"%s\" but no such field is accessible within its MiningSchema's active context %s or Output section %s" % (self.__class__.__name__, self.attrib["field"], dataContext.contextString(), pmmlOutput.featuredFields.keys())
            else:
                raise XMLValidationError, "%s references field \"%s\" but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["field"], dataContext.contextString())
        ...
        if "mapMissingTo" in self.attrib:
            try:
                ...
                raise XMLValidationError, "Could not cast FieldRef mapMissingTo \"%s\": %s" % (self.attrib["mapMissingTo"], str(err))
        else:
...
class NormContinuous(PMML):
    ...
    def post_validate(self):
        ...
        if origs != origs2 or len(origs) != len(set(origs2)):
            raise XMLValidationError, "LinearNorms in NormContinuous must be strictly increasing by \"orig\" (no duplicates, no transpositions)"
        ...
        for index, row in enumerate(self.table.rows):
            for column in columnsToCheck:
                try:
                    ...
                    raise XMLValidationError, "Column \"%s\" missing from row %d of MapValues table%s" % (column, index, self.fileAndLine("<PMML>"))
                ...
                try:
                    ...
                    raise XMLValidationError, "Column \"%s\" in row %d of MapValues table%s has no text" % (column, index, self.fileAndLine("<PMML>"))
...
class Aggregate(PMML):
        ...
        if self.sqlWhere is None:
            ...
            if m is not None:
                try:
                    ...
                    raise XMLValidationError, "Illegal expression in sqlWhere: \"%s\" (%s)" % (self.sqlWhere, str(err))
            ...
            raise XMLValidationError, "Illegal expression in sqlWhere: \"%s\" (doesn't match any SQL WHERE patterns)" % (self.sqlWhere)
    ...
    def top_validate_output(self, dataContext, pmmlOutput):
        ...
        if "groupField" in self.attrib and self.attrib["groupField"] not in dataContext.cast:
            raise XMLValidationError, "%s references groupField \"%s\" but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["field"], dataContext.contextString())
...
class DataDictionary(PMML):
    ...
    def post_validate(self):
        if "numberOfFields" in self.attrib and len(self.matches(DataField)) != self.attrib["numberOfFields"]:
            raise XMLValidationError, "DataDictionary numberOfFields is %d but the number of DataFields is %d" % (self.attrib["numberOfFields"], len(self.matches(DataField)))

        self.dataFields = {}
        for dataField in self.matches(DataField):
            name = dataField.attrib["name"]
            if name in self.dataFields:
                raise XMLValidationError, "DataField \"%s\" appears more than once" % name
...
class DataField(PMML):
    ...
    def post_validate(self):
        if self.attrib["optype"] == "ordinal" and self.attrib["dataType"] == "string":
            ...
            if len(values) == 0:
                raise XMLValidationError, "DataFields with ordinal optypes and string dataTypes are required to have enumerated Values"
...
class Interval(PMML):
    ...
    def post_validate(self):
        if "leftMargin" not in self.attrib and "rightMargin" not in self.attrib:
            raise XMLValidationError, "In an Interval, \"leftMargin\", \"rightMargin\", or both must be specified"
        ...
        if self.leftMargin >= self.rightMargin:
            raise XMLValidationError, "In an Interval, the leftMargin must be less than the rightMargin"
    ...
    def top_validate_transformationDictionary(self, transformationDictionary):
        ...
        if function in TransformationDictionary.builtin:
            ...
            if not variableParameters and minParameters != len(self.arglist):
                raise XMLValidationError, "Built-in function \"%s\" takes exactly %d arguments (%d applied)" % (function, minParameters, len(self.arglist))

            if minParameters > len(self.arglist):
                raise XMLValidationError, "Built-in function \"%s\" takes at least %d arguments (%d applied)" % (function, minParameters, len(self.arglist))
            ...
            if len(self.function.parameters) != len(self.arglist):
                raise XMLValidationError, "User-defined function \"%s\" takes exactly %d arguments (%d applied)" % (function, len(self.function.parameters), len(self.arglist))

        else:
            raise XMLValidationError, "Apply function \"%s\" not recognized (not built-in and not user-defined)" % function
...
class Extension(PMML):
    ...
    def extender(self, name):
        if "extender" in self.attrib:
            self.attrib["extender"] = "%s, %s" % (self.attrib["extender"], name)
        else:
            self.attrib["extender"] = name
...
class Array(PMML):
    ...
    def post_validate(self):
        ...
        if len(xmlText) == 0 and hasattr(self, "value"):
            ...
            raise XMLValidationError, "Array is in an inconsistent state: it has a 'values' member (Pythonic) and it contains text (XMLian)"

        if "n" in self.attrib:
            if self.attrib["n"] != len(self.value):
                raise XMLValidationError, "Array[\"n\"] should be equal to len(Array.values)"
        else:
    ...
    def post_validate(self):
        ...
        if self.attrib["kind"] == "diagonal":
            if self.exists(MatCell) or len(self.matches(Array)) != 1:
                raise XMLValidationError, "Diagonal matrix should be a single Array (no MatCells)"
            ...
            if self.attrib["nbRows"] != length or self.attrib["nbCols"] != length:
                raise XMLValidationError, "Diagonal matrix represented by %d-element Array, yet nbRows, nbCols are %d, %d" % (length, self.attrib["nbRows"], self.attrib["nbCols"])
            ...
            if self.exists(MatCell):
                raise XMLValidationError, "Symmetric matrix should be a sequence of Arrays (no MatCells)"

            arrays = self.matches("Array")
            for i, array in enumerate(arrays):
                if i != len(array.value):
                    raise XMLValidationError, "Symmetric matrix should be a sequence of incrementally growing Arrays: row %d has a %d-length Array" % (i, len(array.value))
            ...
            if self.attrib["nbRows"] != length or self.attrib["nbCols"] != i:
                raise XMLValidationError, "Symmetric matrix represented by an incrementally growing sequence of Arrays, ending with length %d, yet nbRows, nbCols are %d, %d" % (i, self.attrib["nbRows"], self.attrib["nbCols"])
            ...
            if len(arrays) > 0 and len(matcells) > 0:
                raise XMLValidationError, "Matrix must consist of Arrays or MatCells, but not both"

            if len(arrays) > 0:
                length = len(arrays[0].value)
                for array in arrays:
                    if len(array.value) != length:
                        raise XMLValidationError, "Matrix must be rectangular (first Array has length %d, but a later one has length %d)" % (length, len(array.value))

                if "nbRows" not in self.attrib: self.attrib["nbRows"] = len(arrays)
                if "nbCols" not in self.attrib: self.attrib["nbCols"] = length
                if self.attrib["nbRows"] != len(arrays) or self.attrib["nbCols"] != length:
                    raise XMLValidationError, "Matrix contains %d Arrays of length %d, yet nbRows, nbCols are %d, %d" % (len(arrays), length, self.attrib["nbRows"], self.attrib["nbCols"])
                ...
                if "nbRows" not in self.attrib or "nbCols" not in self.attrib:
                    raise XMLValidationError, "Matrix is sparsely defined by MatCells, so nbRows, nbCols must be set explicitly"

                if "diagDefault" not in self.attrib or "offDiagDefault" not in self.attrib:
                    raise XMLValidationError, "Matrix is sparsely defined by MatCells, so diagDefault, offDiagDefault must be set explicitly"

                self.lookup = {}
                for matcell in matcells:
                    index = matcell.attrib["row"] - 1, matcell.attrib["col"] - 1
                    if index in self.lookup:
                        raise XMLValidationError, "More than one MatCell is occupying row %d, col %d" % (matcell.attrib["row"], matcell.attrib["col"])
                    if index[0] < 0 or index[0] >= self.attrib["nbRows"] or index[1] < 0 or index[1] >= self.attrib["nbCols"]:
                        raise XMLValidationError, "MatCell at %d, %d is out of range (nbRows, nbCols is %d, %d)" % (matcell.attrib["row"], matcell.attrib["col"], self.attrib["nbRows"], self.attrib["nbCols"])
    ...
    def post_validate(self):
        ...
        for miningField in self.matches(MiningField):
            ...
            if name in self.miningFields:
                raise XMLValidationError, "MiningField \"%s\" appears more than once in the same MiningSchema" % name
        ...
        for miningField in self.matches(MiningField):
            ...
            if parentContext is None:
                ...
                if required and name not in parentContext_names:
                    raise XMLValidationError, "Nested MiningSchema %s field \"%s\" not found in the DataDictionary or parent context" % (usageType, name)
    ...
    def post_validate(self):
        if "outliers" in self.attrib and self.attrib["outliers"] in ("asMissingValues", "asExtremeValues"):
            if "lowValue" not in self.attrib or "highValue" not in self.attrib:
                raise XMLValidationError, "MiningField with outliers == \"asMissingValues\" or \"asExtremeValues\" requires a lowValue and a highValue"
...
class GeneralRegressionModel(PMML):
    ...
    def top_validate(self, dataContext):
        if "targetVariableName" in self.attrib and self.attrib["targetVariableName"] not in dataContext.cast:
            raise XMLValidationError, "%s references field \"%s\" (as targetVariableName) but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["targetVariableName"], dataContext.contextString())

        if "trialsVariable" in self.attrib and self.attrib["trialsVariable"] not in dataContext.cast:
            raise XMLValidationError, "%s references field \"%s\" (as trialsVariable) but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["trialsVariable"], dataContext.contextString())

        if "offsetVariable" in self.attrib and self.attrib["offsetVariable"] not in dataContext.cast:
            raise XMLValidationError, "%s references field \"%s\" (as offsetVariable) but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["offsetVariable"], dataContext.contextString())

        if "endTimeVariable" in self.attrib and self.attrib["endTimeVariable"] not in dataContext.cast:
            raise XMLValidationError, "%s references field \"%s\" (as endTimeVariable) but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["endTimeVariable"], dataContext.contextString())

        if "startTimeVariable" in self.attrib and self.attrib["startTimeVariable"] not in dataContext.cast:
            raise XMLValidationError, "%s references field \"%s\" (as startTimeVariable) but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["startTimeVariable"], dataContext.contextString())

        if "subjectIDVariable" in self.attrib and self.attrib["subjectIDVariable"] not in dataContext.cast:
            raise XMLValidationError, "%s references field \"%s\" (as subjectIDVariable) but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["subjectIDVariable"], dataContext.contextString())

        if "statusVariable" in self.attrib and self.attrib["statusVariable"] not in dataContext.cast:
            raise XMLValidationError, "%s references field \"%s\" (as statusVariable) but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["statusVariable"], dataContext.contextString())

        if "baselineStrataVariable" in self.attrib and self.attrib["baselineStrataVariable"] not in dataContext.cast:
            raise XMLValidationError, "%s references field \"%s\" (as baselineStrataVariable) but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["baselineStrataVariable"], dataContext.contextString())
...
class PPCell(PMML):
    ...
    def top_validate(self, dataContext):
        if self.attrib["predictorName"] not in dataContext.cast:
            raise XMLValidationError, "%s references field \"%s\" but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["predictorName"], dataContext.contextString())
...
class TreeModel(PMML):
    ...
    def post_validate(self):
            ...
            for node in nodes:
                try:
                    ...
                    if "id" in node.attrib:
                        raise XMLValidationError, "If the TreeModel's functionName is 'regression', Nodes must have numerical scores; Node \"%s\"%s does not" % (node.attrib["id"], node.fileAndLine())
                    else:
                        raise XMLValidationError, "If the TreeModel's functionName is 'regression', Nodes must have numerical scores; Node (no id)%s does not" % (node.attrib["id"], node.fileAndLine())
        ...
        for node in nodes:
            if "id" in node.attrib:
                ...
                if name not in usedNames:
                    ...
                    raise XMLValidationError, "Node id=\"%s\" appears more than once" % name
        ...
        if self.attrib.get("splitCharacteristic", "multiSplit") == "binarySplit":
            for node in nodes:
                if len(node.subnodes) not in (0, 2):
                    raise XMLValidationError, "TreeModel splitCharacteristic is 'binarySplit', but node \"%s\" has %d sub-nodes" % (node.attrib["id"], len(node.subnodes))
            ...
            for node in nodes:
                if len(node.subnodes) != 0:
                    if "defaultChild" not in node.attrib:
                        raise XMLValidationError, "TreeModel missingValueStrategy is 'defaultChild', but node \"%s\" does not have a defaultChild attribute" % node.attrib["id"]
        ...
        if not isinstance(self.node.predicate, pmmlTrue):
            raise XMLValidationError, "Top-level node of TreeModel must have <True /> as its predicate, not %s" % repr(self.node.predicate)  # FIXME: is that correct?
...
class Node(PMML):
    ...
    def calculateProbabilities(self):
        ...
        sumOfProbabilities = sum(self.scoreDistribution.values())
        if abs(sumOfProbabilities - 1.) > 1e-5:
            raise XMLValidationError, "Probabilities of ScoreDistributions in Node do not add to 1.0, they add to %g (are they a mixture of user-specified \"probability\" and probabilities calculated from \"recordCount\"?)" % sumOfProbabilities
    ...
    def post_validate(self):
        ...
        if self.score is None:
            raise XMLValidationError, "Node%s has no score attribute and no ScoreDistribution" % (" %s" % self.attrib["id"] if "id" in self.attrib else "")
        ...
        if "defaultChild" in self.attrib:
            ...
            if self.defaultChild is None:
                raise XMLValidationError, "Node%s has defaultChild='%s' but none of its immediate children have that id" % (" %s" % self.attrib["id"] if "id" in self.attrib else "", self.attrib["defaultChild"])
...
class SimplePredicate(PMML):
    ...
    def post_validate(self):
        ...
        if self.needsValue and "value" not in self.attrib:
            raise XMLValidationError, "SimplePredicate must have a value unless operator is \"isMissing\" or \"isNotMissing\""
    ...
    def top_validate(self, dataContext):
        ...
        if "value" in self.attrib:
            try:
                ...
                raise XMLValidationError, "Could not cast SimplePredicate value \"%s\": %s" % (self.attrib["value"], str(err))
...
class BaselineModel(PMML):
    ...
    def post_validate(self):
        self.isScorable = self.attrib.get("isScorable", True)
        if self.attrib["functionName"] != "regression":
            raise XMLValidationError, "The only valid BaselineModel functionNames are: 'regression'"
...
class TestDistributions(PMML):
    ...
    def post_validate(self):
        ...
        if self.attrib["testStatistic"] == "CUSUM":
            ...
            if not self.exists(Alternate):
                raise XMLValidationError, "Alternate distribution must be supplied if TestDistributions \"testStatistic\" == \"CUSUM\""
        ...
        if self.attrib["testStatistic"] in ("chiSquareDistribution", "scalarProduct"):
            ...
            if table is None:
                raise XMLValidationError, "TestDistributions \"%s\" requires either a CountTable or a NormalizedCountTable" % self.attrib["testStatistic"]
            for child in table.matches(nonExtension):
                if not isinstance(child, FieldValueCount):
                    raise XMLValidationError, "TestDistributions \"%s\" requires a one-dimensional count table (only FieldValueCounts)" % self.attrib["testStatistic"]

        ### attributes

        if "weightField" in self.attrib and self.attrib["testStatistic"] not in ("scalarProduct", "chiSquareDistribution"):
            raise XMLValidationError, "TestDistributions \"weightField\" can only be supplied if \"testStatistic\" in (\"scalarProduct\", \"chiSquareDistribution\")"

        if "normalizationScheme" in self.attrib:
            if self.attrib["testStatistic"] != "scalarProduct":
                raise XMLValidationError, "TestDistributions \"normalizationScheme\" can only be supplied if \"testStatistic\" == \"scalarProduct\""

            if self.attrib["normalizationScheme"] not in ("Independent", "SizeWeighted"):
                raise XMLValidationError, "TestDistributions \"normalizationScheme\" is not in (\"Independent\", \"SizeWeighted\")"
    ...
    def top_validate(self, dataContext):
        ...
        if "weightField" in self.attrib and self.attrib["weightField"] not in dataContext.cast:
            raise XMLValidationError, "%s references field \"%s\" but no such field is accessible within its MiningSchema's active context %s" % (self.__class__.__name__, self.attrib["weightField"], dataContext.contextString())
    ...
    def post_validate(self):
        ...
        for node in nonExtensions:
            if node.attrib["field"] != first_field:
                raise XMLValidationError, "All children of a FieldValue must have the same field name"
...
class NormalizedCountTable(PMML):
    ...
    def post_validate(self):
        ...
        if "sample" not in self.attrib:
            ...
            if any_nonzero:
                raise XMLValidationError, "NormalizedCountTable has non-zero FieldValueCounts but no \"sample\" attribute to know how they were normalized"
...
class FieldValue(PMML):
    ...
    def top_validate(self, dataContext):
        ...
        try:
            ...
            raise XMLValidationError, "Could not cast FieldValue value \"%s\": %s" % (self.attrib["value"], str(err))
...
""")
    ...
    def top_validate(self, dataContext):
        ...
        try:
            ...
            raise XMLValidationError, "Could not cast FieldValueCount value \"%s\": %s" % (self.attrib["value"], str(err))
  ...
  """)
    ...
    def post_validate(self):
        ...
        if len(theFlags) > identified:
            raise XMLValidationError, "Unrecognized regular expression flag in \"%s\"" % theFlags

        try:
            self.pattern = re.compile(self.attrib["pattern"], flags)
        except sre_constants.error, err:
            raise XMLValidationError, "Failed to parse regular expression \"%s\"%s: %s" % (self.attrib["pattern"], "" if theFlags == "" else (" (flags \"%s\")" % theFlags), str(err))
...
class X_ODG_AggregateReduce(X_ODG_PMML):
  ...
  """)
    ...
    def top_validate(self, dataContext):
        ...
        if self.derivedField is None:
            raise XMLValidationError, "%s references derived field \"%s\" but no such derived field is accessible within its MiningSchema's active context {%s}" % (self.__class__.__name__, self.attrib["field"], dataContext.contextString(derived=True))
    ...
    def post_validate(self):
        ...
        if self.attrib["function"] == "python":
            if "pythonExpression" not in self.attrib:
                raise XMLValidationError, "If X-ODG-Apply has function=\"python\", then it must have a \"pythonExpression\""
            else:
            ...
            if self.expression is not None:
                if self.exists(lambda x: isExpression(x) and not isinstance(x, FieldRef)):
                    raise XMLValidationError, "If X-ODG-Apply has a \"pythonExpression\", then it can only contain FieldRefs"
                ...
                if "pythonLibraries" in self.attrib:
                    for library in re.split("[\s,;]+", self.attrib["pythonLibraries"].lstrip(" \t\n\r").rstrip(" \t\n\r")):
                        ...
                        try:
                            ...
                            raise XMLValidationError, "Could not load library \"%s\"" % library
    ...
    def post_validate(self):
        ...
        for c in self.matches(X_ODG_CustomProcessingConstant):
            ...
            if name in namesSeen:
                raise XMLValidationError, "Multiple X-ODG-CustomProcessingConstants with the name \"%s\"" % name
            namesSeen.add(name)
  ...
  """)
    ...
    def post_validate(self):
        ...
        if self.ignore is not None:
            try:
                ...
                raise XMLValidationError, "Attribute 'ignore' is not a regular expression: \"%s\"" % self.ignore
        ...
        if self.nGramGroups is not None and self.delimiter is None:
            raise XMLValidationError, "You can only group words into n-grams if you have first grouped characters into words with a delimiter"
    ...
    def top_validate(self, dataContext):
        ...
        if dataContext.cast[self.attrib["field"]] != pmmlBuiltinType["string"]:
            raise XMLValidationError, "CountSubstrings field \"%s\" must be a string in its MiningSchema context; instead, it is \"%s\"" % (self.attrib["field"], str(dataContext.cast[self.attrib["field"]]))

...
if len(logging._handlers) == 0: log.addHandler(logging.StreamHandler())
...
class TableMethods(object):
  ...
  def _from_nab(self,fd,header=None,**kwargs):
    ...
    try:
      ...
      accum = None
    # TANYA:  The only errors that numpy.core.records.fromfile raise are ValueError and IOError; commenting out the below...
    #except self.BufferError:
  ...
  def from_csv_file(self,filename,header=None,insep=None,**kwargs):
    '''read CSV from file'''
    return self._process_file(filename,'rbU',self._from_csv,header=header,insep=insep,**kwargs)
  ...
  def _from_csv(self, fd, header=None, **kwargs):
    ...
    keys = csv.reader(StringIO(str(header)),dialect=dialect).next()
    self.setkeys(keys)
